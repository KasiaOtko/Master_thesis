...
def train_GCN(config):
    ...
    if "ogb" in hparams.dataset.name:
        ...
        if hparams["model"] == "gcn":
            ...
            model = GATv2(data.x.shape[1], hparams["hidden_channels"], num_classes, hparams["dropout_p"], hparams["heads"],
                        hparams["num_layers"], hparams["linear"]).to(device)
        elif hparams["model"] == "sage":
    ...
    log_details_to_wandb(hparams["model"], hparams, n_trainable_params)
